...
/// 公有字段
/// </summary>
public partial class Star3
{
    public bool sophisticated = false;                 //圆滑曲面
    public bool star = true;                                //星形
    public int line = 5;                                                //边数
    [SerializeField]
    private float[] maxsize = { 5, 5, 5, 5, 5 };    //外角大小
    [SerializeField]
    private float[] minSize = { 1, 1, 1, 1, 1 };     //内角大小
    public float high = 1;                                            //高度
    public float low = -1;                                            //低度
}

public partial class Star3 : MonoBehaviour
{
    Mesh mesh;
    ...
    void Update()
    {
        ...
        for (int i = 1; i < maxsize.Length - 1; i++)
        {
